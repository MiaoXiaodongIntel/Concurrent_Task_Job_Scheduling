...
Covers design_control_plane.md §3.6 and design_task_manager.md §2.2 transition 7:
- abort a running task → accepted, status=aborted, abort_reason=user_abort
- abort a non-running task → rejected with task_not_running
...
def test_abort_running_task_accepted(tmp_path):
    ...
    task = TaskJob(task_id="t1", commands=["echo hi"])
...
def test_abort_non_running_task_rejected(tmp_path):
    """abort_task on a QUEUED task is rejected with task_not_running."""
    task = TaskJob(task_id="t1", commands=["echo hi"])
    manager = _make_manager(tmp_path, [task])
    ...
    assert result["reason_code"] == "task_not_running"
...
def test_abort_task_sibling_unaffected(tmp_path):
    ...
    t1 = TaskJob(task_id="t1", commands=["echo hi"])
    t2 = TaskJob(task_id="t2", commands=["echo hi"])
...
def test_abort_aborted_task_rejected(tmp_path):
    """abort_task on an already-aborted task is rejected with task_not_running."""
    task = TaskJob(task_id="t1", commands=["echo hi"])
    manager = _make_manager(tmp_path, [task])
    with manager._lock:
