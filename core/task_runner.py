...
def build_powershell_script(commands: list[str]) -> str:
    ...
    parts: list[str] = [
        ...
        "    }",
        "",
        "    # Run each step in the same PowerShell process/session. Wrap in",
        "    # try/catch so a terminating error (ErrorActionPreference = 'Stop')",
        "    # is reported as a single clean stderr line instead of PowerShell's",
        "    # default multi-line ErrorRecord, which gets wrapped/garbled when",
        "    # stdout/stderr are redirected to pipes (no real console width).",
        "    try {",
        "        Invoke-Expression $Command",
        "    }",
        "    catch {",
        "        $message = $_.Exception.Message",
        "        [Console]::Error.WriteLine((\"Step {0} failed: {1}\" -f $StepNo, $message))",
        "        exit 1",
        "    }",
        "",
        "    $lastExit = $null",
        ...
        "        # Use the raw error stream (not Write-Error) so the message is not",
        "        # turned into a terminating error and is not reformatted/wrapped;",
        "        # this also lets the true exit code propagate via 'exit $lastExit'.",
        "        [Console]::Error.WriteLine((\"Step {0} failed with exit code {1}\" -f $StepNo, $lastExit))",
        "        exit $lastExit",
        "    }",
        "}",
        "",
